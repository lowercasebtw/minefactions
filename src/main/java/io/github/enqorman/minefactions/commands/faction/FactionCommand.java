    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String name, @NotNull String[] args) {
        ...
        if (args.length > 0) {
            ...
            if (subCommand.execute(sender, Arrays.copyOfRange(args, 1, args.length)) == CommandResult.FAIL) {
                MineFactionsPlugin.getInstance().getLogger().log(Level.SEVERE, "Failed to execute subcommand " + cmdName + " for faction command!");
                return true;
            }
        ...
        sender.sendMessage("TODO: commands list");
        return true;
    }
}
