...
public class Faction {
	...
	public ConfigurationSection saveFaction(ConfigurationSection factionsSection) {
		...
		if (homeLocation != null) {
			...
			section.set("home", SerializeUtil.saveLocation(homeLocation, homeSection)); // Will become blank if location fails to save
		}
		
		if (boundingBox != null) {
			ConfigurationSection claimSection = section.createSection("claim");
			section.set("claim", SerializeUtil.saveBoundingBox(this.boundingBox, claimSection)); // Will become blank if bounding box fails to save
		}
		
		return factionsSection;
	}
	
	public static Faction loadFaction(ConfigurationSection section) throws Exception {
		...
		if (homeSection != null) {
			homeLocation = SerializeUtil.loadLocation(homeSection); // Will be null if it fails to load
		}
		
		ConfigurationSection claimSection = section.getConfigurationSection("claim");
		BoundingBox boundingBox = null;
		if (claimSection != null) {
			boundingBox = SerializeUtil.loadBoundingBox(claimSection); // Will be null if it fails to load
		}
